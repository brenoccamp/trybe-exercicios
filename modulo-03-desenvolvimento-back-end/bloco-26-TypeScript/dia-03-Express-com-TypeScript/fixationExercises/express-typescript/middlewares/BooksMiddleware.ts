...
function validationBook(req: Request, res: Response, next: NextFunction): Response | void {
  ...
  if (!valid) {
    return res.status(StatusCodes.BAD_REQUEST).send(`O campo ${property} não pode ser vazio.`);
  }
